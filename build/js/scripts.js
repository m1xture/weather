...
unitsBtns.addEventListener("click", function (e) {
  if (e.target.hasAttribute("data-f")) {
    selectedUnits = "F";
    e.target.classList.remove("degrees__disable");
    e.target.nextElementSibling.classList.add("degrees__disable");
  } else {
    selectedUnits = "C";
    e.target.classList.remove("degrees__disable");
    e.target.previousElementSibling.classList.add("degrees__disable");
  }
  weatherFetch.call(formEl);
...
if (localStorage.getItem("cityValue")) {
  formEl.cityInput.value = localStorage.getItem("cityValue");
}
...
function weatherFetch() {
  fetch(
    ...
    .then(function (data) {
      ...
      localStorage.setItem("cityValue", formEl.cityInput.value);
      ...
      if (data.weather[0].main === "Clouds") {
        if (data.weather[0].icon === "02d") {
          ...
          weatherElems.currentWeatherIcon.src = "../img/icons/cloudsMoonN.png";
      ...
      if (data.weather[0].main === "Rain") {
        if (
          ...
          weatherElems.currentWeatherIcon.src = "../img/icons/cloudRainDeg.png";
        } else if (
      ...
      fetch(
        ...
        .then((dataForecast) => {
          ...
          if (selectedUnits === "F") {
            weatherElems.firstDay.firstDayOfWeekDegreeEl.textContent =
              dataForecast.forecast.forecastday[1].day.avgtemp_f.toFixed();
            weatherElems.secondDay.secondDayOfWeekDegreeEl.textContent =
              dataForecast.forecast.forecastday[2].day.avgtemp_f.toFixed();
            weatherElems.thirdDay.thirdDayOfWeekDegreeEl.textContent =
              dataForecast.forecast.forecastday[3].day.avgtemp_f.toFixed();
          }
              ...
              allDaysElems.map((iconEl) => {
                ...
                allStatuses.map((status) => {
                  if (
                    ...
                    switch (
                      dataForecast.forecast.forecastday[acc].day.condition.text
                    ) {
                      case "Sunny":
